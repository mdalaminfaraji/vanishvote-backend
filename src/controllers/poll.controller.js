...
exports.getPollById = async (req, res, next) => {
  try {
    ...
    if (Date.now() > new Date(poll.expiresAt).getTime()) {
      return next(createError(410, 'This poll has expired and is no longer available'));
    }
...
exports.votePoll = async (req, res, next) => {
  try {
    ...
    // Check if poll has expired
    if (Date.now() > new Date(poll.expiresAt).getTime()) {
      return next(createError(410, 'This poll has expired and votes are no longer accepted'));
    }
...
exports.addReaction = async (req, res, next) => {
  try {
    ...
    if (!reactionType || !['trending', 'like'].includes(reactionType)) {
      return next(createError(400, 'Invalid reaction type. Must be "trending" or "like"'));
    }
    ...
    // Check if poll has expired
    if (Date.now() > new Date(poll.expiresAt).getTime()) {
      return next(createError(410, 'This poll has expired and reactions are no longer accepted'));
    }
...
exports.getPollResults = async (req, res, next) => {
  try {
    ...
    if (poll.hideResults && Date.now() < new Date(poll.expiresAt).getTime()) {
      return next(createError(403, 'Results for this poll are hidden until it expires'));
    }
